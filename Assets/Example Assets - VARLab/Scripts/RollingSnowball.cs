...
namespace TigerTail
{
    ...
    public class RollingSnowball : MonoBehaviour
    {
        ...
        {
            if (Physics.Raycast(transform.position, Vector3.down, out RaycastHit hit, collider.radius * currentSize + 0.5f))
            {
                particles.transform.SetPositionAndRotation(hit.point, Quaternion.identity);
            }
        }

        private void FixedUpdate()
        {
            // Add our current angular speed to the number of completed rotations.
            completedRotations += rigidbody.angularVelocity.magnitude * Time.fixedDeltaTime; 
        }

        private void CalculateMass()
        {
            const float SPHERE_VOLUME_CONSTANT = 4 * Mathf.PI / 3;
            var r = collider.radius * currentSize;
            rigidbody.mass = DENSITY_OF_SNOWBALL * SPHERE_VOLUME_CONSTANT * r * r * r; // Multiplying by r 3 times is faster than Mathf.Pow.
        }
